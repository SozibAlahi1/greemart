...
export async function POST(request: NextRequest) {
  try {
    ...
    const appleProduct = createdProducts.find((p) => p.name === 'Fresh Organic Apples');
    const milkProduct = createdProducts.find((p) => p.name === 'Whole Milk');
    const salmonProduct = createdProducts.find((p) => p.name === 'Fresh Salmon Fillet');
    const breadProduct = createdProducts.find((p) => p.name === 'Artisan Bread');
    const juiceProduct = createdProducts.find((p) => p.name === 'Orange Juice');

    // Create reviews
    if (appleProduct) {
      await Review.insertMany([
        {
          productId: appleProduct._id,
